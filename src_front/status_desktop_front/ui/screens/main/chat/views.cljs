(ns status-desktop-front.ui.screens.main.chat.views
...
(views/defview member-photo [from]
  (views/letsubs [photo-path [:photo-path from]]
    [react/view
...
(views/defview messages-view [{:keys [chat-id group-chat]}]
  (views/letsubs [chat-id* (atom nil)
    ...
    (let [_ (when (or (not @chat-id*) (not= @chat-id* chat-id))
      ...
      [react/view {:style {:flex 1 :background-color :white :margin-horizontal 16}};"#eef2f5"}}
...
(views/defview chat-view []
  (views/letsubs [current-chat [:get-current-chat]]
    [react/view {:style {:flex 1 :background-color :white}};"#eef2f5"}}
     [toolbar-chat-view]
